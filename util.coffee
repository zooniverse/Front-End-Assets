define (require, exports, module) ->
  ...
  dateWords =
    ...
    y: (date) -> "#{date.getUTCFullYear()}"[...2] # 84
    month: (date) -> months[date.getUTCMonth()] # February
    mon: (date) -> months[date.getUTCMonth()][...3] # Feb
    m: (date) -> date.getUTCMonth() + 1 # 2
    date: (date) -> "#{date.getUTCDate()}" # 25
    day: (date) -> days[date.getUTCDay()] # Thursday
    d: (date) -> days[date.getUTCDay()][...3] # Thu
    time24: (date) -> "#{date.getUTCHours()}:#{('0' + date.getUTCMinutes())[-2...]}" # 15:00
    time12: (date) -> "#{(((date.getUTCHours() % 12) + 12) % 12)}:#{('0' + date.getUTCMinutes())[-2...]}" # 3:00
  ...
  module.exports =
    arraysMatch: (first, second) ->
      ...
      for item, i in first
        if first[i] instanceof $
          return false unless first[i].is second[i]
        # else if first[i] instanceof Spine.Model
    ...
    remove: (thing, {from: array}) ->
      for something, i in array
        ...
        # else if thing instanceof Spine.Model
